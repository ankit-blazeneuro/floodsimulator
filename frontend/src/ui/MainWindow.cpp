...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent) {
    ...
    // Initialize Indian Dam dataset FIRST so all child screens receive all 6,600+ dams
    damManager = std::make_unique<MapCore::DamManager>();
    damManager->loadFromGeoJson("server/dataset/dam.geojson");
    ...
    // Attach dam dataset to online map, search, and screens
    ...
    if (monitorScreen) {
        monitorScreen->setDamManager(damManager.get());
    }
...
void MainWindow::setupMenuBar() {
    ...
    btnWorkspaceSim->setToolTip("Simulation Workspace (Map & Hydrodynamics)");
    ...
    wsButtonGroup->addButton(btnWorkspaceHelp, 2);
    topBarLayout->addWidget(btnWorkspaceHelp);

    // Monitor Workspace Button (Icon + Title "Monitor", Emerald/Cyan Theme)
    btnWorkspaceMonitor = new QPushButton(" Monitor", topBarWidget);
    btnWorkspaceMonitor->setObjectName("btnWorkspaceMonitor");
    btnWorkspaceMonitor->setIcon(IconHelper::radar(QColor(16, 185, 129), 16));
    btnWorkspaceMonitor->setIconSize(QSize(16, 16));
    btnWorkspaceMonitor->setCheckable(true);
    btnWorkspaceMonitor->setChecked(false);
    btnWorkspaceMonitor->setToolTip("Real-Time Predictive Dam Breach Monitoring (BG_model)");
    wsButtonGroup->addButton(btnWorkspaceMonitor, 3);
    topBarLayout->addWidget(btnWorkspaceMonitor);
    ...
    connect(btnWorkspaceMonitor, &QPushButton::clicked, this, &MainWindow::showMonitorScreen);
...
void MainWindow::setupUi() {
    ...
    // Multi-Screen Root Stack (Index 0: Simulation Workspace, Index 1: Analytics Screen, Index 2: Help & Support Screen, Index 3: Monitor Screen)
    analyticsScreen = new AnalyticsWidget(this);

    helpScreen = new HelpSupportWidget(this);
    monitorScreen = new MonitorWidget(damManager.get(), this);

    rootStack = new QStackedWidget(this);
    rootStack->addWidget(mainSplitter);      // 0: Simulation Map
    rootStack->addWidget(analyticsScreen);   // 1: Analytics
    rootStack->addWidget(helpScreen);        // 2: Help & Support
    rootStack->addWidget(monitorScreen);     // 3: Monitor Screen (BG_model)
    ...
    auto syncAll = [this](int sourceScreenIndex, double lat, double lon, int zoom) {
        ...
        if (sourceScreenIndex != 2 && helpScreen) {
            helpScreen->setViewport(lat, lon, zoom);
        }
        if (sourceScreenIndex != 3 && monitorScreen) {
            monitorScreen->setViewport(lat, lon, zoom);
        }
    ...
    connect(monitorScreen, &MonitorWidget::viewportChanged, this, [syncAll](double lat, double lon, int zoom) {
    ...
    connect(zoomInShortcut, &QShortcut::activated, this, [this]() {
        if (rootStack && rootStack->currentIndex() == 2 && helpScreen) {
            helpScreen->zoomIn();
            return;
        }
...
void MainWindow::showSimulationScreen() {
    ...
    if (onlineMap) {
        onlineMap->setHydroFlowMode(false);
    }
    updateFloatingPositions();
}
...
void MainWindow::showMonitorScreen() {
    double lat = 22.0, lon = 79.0;
    int zoom = 5;
    ...
    if (btnWorkspaceMonitor) {
        btnWorkspaceMonitor->setChecked(true);
    }
}
...
void MainWindow::onCurrentScreenChanged(int index) {
    ...
    // 2. Help & Support Screen (Pause ADS-B/OpenSky live 3s network poller and radar animations when inactive)
    if (helpScreen) {
