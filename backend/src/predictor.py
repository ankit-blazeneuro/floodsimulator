...
class DamBreachPredictor:
    ...
    def __init__(self, model_dir: str = None):
        if model_dir is None:
            # Default: look for models/ relative to the package root
        ...
        for path in [model_path, os.path.join(self.model_dir, "dam_failure_xgb.pkl")]:
            if os.path.exists(path):
                try:
                    ...
                    try:
                        for name, est in getattr(self.model, "named_estimators_", {}).items():
    ...
    def _predict_ml(self, feature_dict: dict) -> float:
        ...
        try:
            ...
            if (
                not feature_dict.get("is_natural", 0)
                and feature_dict.get("freeboard_remaining_m", 5.0) >= 3.0
                and feature_dict.get("rain_1h_mm", 0.0) < 8.0
                and feature_dict.get("rain_24h_mm", 0.0) < 35.0
                and feature_dict.get("crest_displacement_mm_yr", 0.0) < 5.0
            ):
                prob = min(prob, 0.05)
    ...
    def _generate_explanation_text(self, top_factors: list, features: dict) -> str:
        ...
        if prob < 0.25:
            fb = features.get('freeboard_remaining_m', 0)
            return f"Low failure risk ({pct:.0f}%). Dam operating safely within structural tolerances with ample freeboard ({fb:.1f}m remaining)."
